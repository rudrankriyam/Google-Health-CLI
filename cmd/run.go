...
Google Health API workbench for humans, scripts, and agents.
...
func (a *app) simpleResource(resource string, args []string) error {
	...
	case "update":
		...
		var value map[string]any
		if resource == "profile" {
			value, err = client.UpdateProfile(context.Background(), body)
		} else {
			value, err = client.UpdateSettings(context.Background(), body)
		}
		if err != nil {
			return err
		}
...
func (a *app) data(args []string) error {
	...
	case "list", "reconcile":
		...
		if args[0] == "reconcile" {
			value, err := client.ReconcileDataPoints(context.Background(), dataType.EndpointName, healthapi.ReconcileOptions{Filter: finalFilter, PageSize: *limit, PageToken: *pageToken})
			if err != nil {
				return err
			}
			return output.Print(a.out, value, a.opts)
	...
	case "export-tcx":
		...
		bytes, err := client.ExportExerciseTCX(context.Background(), args[1])
		if err != nil {
			return err
		}
...
func (a *app) rollup(args []string) error {
	...
	if *file != "" {
		...
		body["filter"] = registry.FilterFromRange(dataType, *from, *to)
	}
	var value map[string]any
	switch mode {
	case "daily":
...
func (a *app) subscribers(args []string) error {
	...
	case "list":
		...
		value, err := client.ListSubscribers(context.Background(), *projectFlag)
		if err != nil {
			return err
		}
		return output.Print(a.out, value, a.opts)
	case "create":
		...
		value, err := client.CreateSubscriber(context.Background(), *projectFlag, body)
		if err != nil {
			return err
		}
		return output.Print(a.out, value, a.opts)
	case "patch":
		...
		value, err := client.PatchSubscriber(context.Background(), *name, body)
		if err != nil {
			return err
		}
		return output.Print(a.out, value, a.opts)
	case "delete":
		...
		value, err := client.DeleteSubscriber(context.Background(), *name)
		if err != nil {
			return err
		}
		return output.Print(a.out, value, a.opts)
...
func agentManifest(version string) map[string]any {
	return map[string]any{
		"name":        "ghealth",
		"version":     version,
		"description": "Google Health API workbench CLI",
		"api": map[string]any{
...
func unwrapUsage(err error) error {
	if errors.Is(err, errUsage) || errors.Is(err, errAuth) {
